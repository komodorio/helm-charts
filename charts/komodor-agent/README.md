...
![AppVersion: 0.2.242](https://img.shields.io/badge/AppVersion-0.2.242-informational?style=flat-square)
...
| publicApiKey | guid | `nil` | Komodor public API key. Required when features that depend on it are enabled (currently capabilities.klaudiaIntegrationSync). |
| publicApiKeySecret | string | `nil` | Existing Kubernetes secret containing the public API key |
| publicApiKeySecretKey | string | `nil` | Key inside publicApiKeySecret (defaults to publicApiKey) |
...
| telegrafImageVersion | string | `"v2.0.85-alpine"` | Telegraf version to be used |
| telegrafWindowsImageVersion | string | `"v2.0.85"` | Telegraf version to be used for windows |
| admissionControllerVersion | string | `"0.1.76"` | Admission controller version to be used |
...
| capabilities.resourceInfo.enabled | bool | `true` | Enable the in-watcher resource-info collector. Komodor's remote config enables this for every agent, so the chart default matches what actually runs; profile=cost turns it off and enforces that with an env var, which remote config cannot override. |
...
| components.komodorAgent.watcher.extraVolumes | list | `[]` | List of additional volumes and their mounts to add to the watcher container. Example (projected ServiceAccount token):      extraVolumes:        - volume:            name: workload-identity-token            projected:              sources:                - serviceAccountToken:                    path: token                    expirationSeconds: 3600                    audience: <token-exchange-audience>          volumeMount:            name: workload-identity-token            mountPath: /var/run/secrets/tokens            readOnly: true |
...
| components.admissionController.spotAvoidance | object | `{"eksManagedNodeGroups":false}` | Extra spot-avoidance guards that are cloud-specific and unsafe by default. |
| components.admissionController.spotAvoidance.eksManagedNodeGroups | bool | `false` | When true, appends `eks.amazonaws.com/capacityType NotIn [SPOT]` to every entry under `affinity.nodeAffinity.preferredDuringSchedulingIgnoredDuringExecution[].preference.matchExpressions` (preferred branch only — has no effect if you override `nodeAffinity` with only `requiredDuringSchedulingIgnoredDuringExecution`; also, expressions within a preference are AND'd, so any custom preferences you set will also require non-SPOT). ONLY safe on EKS clusters using Managed Node Groups — set true to preserve the pre-fix behavior on MNG clusters. MUST remain false on EKS Auto Mode: `eks.amazonaws.com` is a restricted label domain there and `capacityType` is not a well-known key, so Karpenter rejects NodeClaim creation and node provisioning halts cluster-wide. |
| components.admissionController.affinity | object | `{"nodeAffinity":{"preferredDuringSchedulingIgnoredDuringExecution":[{"preference":{"matchExpressions":[{"key":"karpenter.sh/capacity-type","operator":"NotIn","values":["spot"]},{"key":"cloud.google.com/gke-spot","operator":"NotIn","values":["true"]},{"key":"cloud.google.com/gke-preemptible","operator":"NotIn","values":["true"]},{"key":"kubernetes.azure.com/scalesetpriority","operator":"NotIn","values":["spot"]}]},"weight":100}]},"podAffinity":{},"podAntiAffinity":{}}` | Set affinity rules for the komodor admission controller deployment. Split into nodeAffinity, podAffinity, and podAntiAffinity. If podAntiAffinity is not set, defaults to preferred pod anti-affinity on hostname |
| components.admissionController.affinity.nodeAffinity | object | `{"preferredDuringSchedulingIgnoredDuringExecution":[{"preference":{"matchExpressions":[{"key":"karpenter.sh/capacity-type","operator":"NotIn","values":["spot"]},{"key":"cloud.google.com/gke-spot","operator":"NotIn","values":["true"]},{"key":"cloud.google.com/gke-preemptible","operator":"NotIn","values":["true"]},{"key":"kubernetes.azure.com/scalesetpriority","operator":"NotIn","values":["spot"]}]},"weight":100}]}` | Set node affinity for the komodor admission controller deployment |
...
| components.komodorMetrics.metrics.image | object | `{"name":"telegraf","tag":"v2.0.85-alpine"}` | Override the komodor agent metrics image name or tag. |
...
| components.komodorDaemon.metrics | object | `{"extraEnvVars":[],"image":{"name":"telegraf","tag":"v2.0.85-alpine"},"quiet":false,"resources":{"limits":{"cpu":1,"memory":"1Gi"},"requests":{"cpu":0.1,"memory":"384Mi"}},"securityContext":{"allowPrivilegeEscalation":false,"capabilities":{"add":[],"drop":["ALL"]},"runAsNonRoot":true},"sidecar":{"enabled":true}}` | Configure the komodor daemon metrics components |
| components.komodorDaemon.metrics.image | object | `{"name":"telegraf","tag":"v2.0.85-alpine"}` | Override the komodor agent metrics image name or tag. |
...
| components.komodorDaemon.opentelemetry.image | object | `{"name":"komodor-otel-collector","tag":"0.1.14"}` | Override the OpenTelemetry collector image name or tag. |
...
| components.komodorDaemon.opentelemetry.logCollection | object | See sub-values | Container-log tailing (filelog receiver) safety controls.  The otel-collector tails every in-scope container log file. Its read throughput equals the log-write throughput of the scoped pods. On nodes whose storage does NOT benefit from the Linux page cache (e.g. VMware datastores, or memory-pressured nodes), those reads land on the backing device as sustained read IOPS. Keep the scope narrow and exclude high-volume namespaces.  NOTE: when otelInit.enabled=true (the default) the live filelog scope is delivered by the Komodor backend (remote configuration), so it is enforced server-side. These chart values apply to the static fallback config used when otelInit.enabled=false. |
| components.komodorDaemon.opentelemetry.logCollection.excludeNamespaces | list | `[]` | Namespaces whose pod logs must never be tailed. |
...
| components.komodorDaemonWindows.metrics | object | `{"extraEnvVars":[],"image":{"name":"telegraf-windows","tag":"v2.0.85"},"quiet":false,"resources":{"limits":{"cpu":1,"memory":"1Gi"},"requests":{"cpu":0.1,"memory":"384Mi"}},"securityContext":{},"sidecar":{"enabled":true}}` | Configure the komodor daemon metrics components |
| components.komodorDaemonWindows.metrics.image | object | `{"name":"telegraf-windows","tag":"v2.0.85"}` | Override the komodor agent metrics image name or tag. |
