...
| `watcher.enableHelmSecretsReporting`               | Enables reporting of unredacted secrets of type "helm.sh/release.v1" containing helm releases data                                                                            | `true`                              |
...
* External proxy (which *cannot* communicate with the local K8s IPs) - You need to use the `KOMOKW_` prefix to the proxy environment variables to have only the traffic to Komodor pass through the proxy. If you're using the Helm chart - this can be disabled by setting `--set proxy.komodorOnly=false`.